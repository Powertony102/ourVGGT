...
def viser_wrapper(
    ...
    if not use_point_map or world_points_map is None or conf_map is None:
        world_points = unproject_depth_map_to_point_map(depth_map, extrinsics_cam, intrinsics_cam)
        conf = depth_conf
        use_point_map = False  # ensure downstream logic matches actual data
    else:
...
def main():
    ...
    if images.shape[-2] % patch_size != 0 or images.shape[-1] % patch_size != 0:
        raise ValueError(
            f"Image dimensions {images.shape[-2:]} must be divisible by patch size {patch_size}"
        )
    patch_height = images.shape[-2] // patch_size
    patch_width = images.shape[-1] // patch_size
    model.update_patch_dimensions(patch_width, patch_height)
    print(f"Updated attention patch grid to {patch_width}x{patch_height}")
    ...
    for key in list(predictions.keys()):
        ...
        if isinstance(value, torch.Tensor):
            ...
            predictions[key] = value.cpu().numpy().squeeze(0)  # remove batch dimension and convert to numpy

    has_world_points = "world_points" in predictions and "world_points_conf" in predictions
    if args.use_point_map and not has_world_points:
        print("Warning: Checkpoint does not provide point map outputs. Falling back to depth-based visualization.")
        args.use_point_map = False
