            ...
            with accelerator.split_between_processes(list(range(len(dataset)))) as idxs:
                for data_idx in tqdm(idxs):
                    ...
                    if model_name == "vggt":
                        ...
                        print(
                            f"[Eval Debug] preds len={len(preds)}, "
                            f"pts_cam shape={pts_cam.shape}, world_conf shape={world_conf.shape}"
                        )
                        ...
                        print(
                            f"[Eval Debug] gt_pts len={len(gt_pts)}, pred_pts len={len(pred_pts)}"
                        )
                        ...
                        for j, view in enumerate(batch):
                            ...
                            if j == 0:
                                print(
                                    f"[Eval Debug] view {j}: image {image.shape}, pts {pts.shape}, "
                                    f"gt_pts {pts_gt.shape}, mask {mask.shape}, conf {conf.shape}"
                                )
                    ...
                    print(
                        f"[Eval Debug] concat shapes -> images {images_all.shape}, "
                        f"pts {pts_all.shape}, pts_gt {pts_gt_all.shape}, masks {masks_all.shape}"
                    )
