...
class NRGBD(BaseStereoViewDataset):
    ...
    def _get_views(self, idx, resolution, rng):
        ...
        if self.tuple_list is not None:
            ...
            # Count only PNG images to avoid non-image files inflating the count
            img_dir = os.path.join(self.ROOT, scene_id, "images")
            num_files = len([f for f in os.listdir(img_dir) if f.lower().endswith('.png')])
