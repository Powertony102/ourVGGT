...
def get_args_parser():
    ...
    parser.add_argument("--lcons", type=float, default=0.5)
    parser.add_argument("--lcov", type=float, default=1.0)
    parser.add_argument("--lbal", type=float, default=1.0)
    parser.add_argument("--partition_groups", type=int, default=-1)
    return parser
...
def main(args):
    ...
    if model_name.upper() == "VGGT":
        part_groups = None if int(getattr(args, "partition_groups", -1)) < 0 else int(getattr(args, "partition_groups", -1))
        model = VGGT(
            merging=args.merging,
            enable_point=True,
            partition_lam=float(getattr(args, "lcons", 0.5)),
            partition_alpha=float(getattr(args, "lcov", 1.0)),
            partition_beta=float(getattr(args, "lbal", 1.0)),
            partition_num_groups=part_groups,
        )
                ...
                if model_name.upper() == "VGGT":
                    ...
                    with torch.cuda.amp.autocast(dtype=dtype_autocast):
                        with torch.no_grad():
                            ...
                            start = time.time()
                            predictions = model(imgs_tensor, num_groups=part_groups)
                            torch.cuda.synchronize()
                            end = time.time()
                            elapsed_s = end - start
